...
//costumer middleware
import {validation,validationRegister,validateLogin} from "../middleware/validator.js"
...
router.post("/registre",validation,validationRegister(), async (req, res) => {
  ...
  try {
    ...
    if (emailFind || usernameFind) {
      return res.status(400).send({ msg: "username or email is already exited !!" });
    }
...
router.post("/login",validateLogin(),validation ,async(req,res)=>{
    ...
    try {
        ...
        if(!userfind){
            return res.status(400).send({ msg: "email or password incorrect '<email>' !!" });
        }
        const isMatch = await bcrypt.compare(password,userfind.password)
        if(!isMatch){
            return res.status(400).send({ msg: "email or password incorrect '<pass>'!!" });
        }
//generate token
